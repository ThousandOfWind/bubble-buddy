...
如果你使用 [GitHub Copilot CLI](https://docs.github.com/copilot/how-tos/copilot-cli),
[支持技能(support skill)](skills/README.md) 可以以对话方式帮你安装和配置
Bubble Buddy。注册一次,然后直接开口即可:

```bash
# 注册 Bubble Buddy 技能(一行命令,无需 clone —— 会下载完整技能)
...
`npx @bubble-buddy/skills` 会下载完整的技能包(SKILL.md 及其全部参考资料)并
注册到你的 Copilot CLI。想更新到最新版本时,重新运行一次即可。(如果 `npx`
暂时找不到该包,可退回下方的[从源码](#面向开发者--从源码运行)方式。)
